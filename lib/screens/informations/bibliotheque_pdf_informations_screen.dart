...
class BibliothequePdfInformationsScreen extends StatefulWidget {
  final DatabaseService databaseService;
  
  const BibliothequePdfInformationsScreen({
    Key? key,
    required this.databaseService,
  }) : super(key: key);
  
  @override
  _BibliothequePdfInformationsScreenState createState() => _BibliothequePdfInformationsScreenState();
}

class _BibliothequePdfInformationsScreenState extends State<BibliothequePdfInformationsScreen> {
  ...
  Future<void> _chargerPdf() async {
    ...
    // Obtenir tous les PDF et filtrer pour ne garder que ceux des informations
    final tousPdf = await widget.databaseService.getListeTousPdf();
    final pdfInfos = tousPdf.where((pdf) => pdf['type'] == 'information').toList();
    
    setState(() {
      _pdfList = pdfInfos;
      _pdfListFiltree = List.from(pdfInfos);
      _isLoading = false;
    });
  }
  ...
  Future<void> _partagerPdf(String filePath, String fileName) async {
    try {
      ...
      await Share.shareXFiles(
        ...
        text: 'Voici le PDF de l\'information "$fileName" généré par JProjets',
      );
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
        ...
        child: Column(
          ...
          children: [
            ...
            Padding(
              padding: const EdgeInsets.symmetric(horizontal: 16),
              child: Text(
                'PDF Information • ${_formatSize(pdf['size'])}',
                style: TextStyle(
                  fontSize: 13,
                    ...
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          ...
                          Text(
                            '${_pdfList.length} information${_pdfList.length > 1 ? 's' : ''} PDF',
                            style: TextStyle(
                              fontSize: 14,
                              color: Colors.white.withOpacity(0.9),
                            ),
                            ...
                            child: Column(
                              ...
                              children: [
                                ...
                                Padding(
                                  padding: const EdgeInsets.symmetric(horizontal: 32),
                                  child: Text(
                                    'Les PDF générés pour vos informations apparaîtront ici',
                                    style: TextStyle(
                                      fontSize: 14,
                                      color: Colors.grey[600],
                                    ),
                                ...
                                ElevatedButton.icon(
                                  ...
                                  label: const Text('Retour aux informations'),
                              ...
                              children: [
                                ...
                                if (!_rechercheActive)
                                  Row(
                                    ...
                                    children: [
                                      Text(
                                        'Informations PDF (${_pdfListFiltree.length})',
                                        style: TextStyle(
                                          fontSize: 18,
                                          fontWeight: FontWeight.w600,
                                          color: Colors.grey[900],
  ...
  Future<void> _supprimerTousPdf() async {
    final confirm = await showDialog(
      context: context,
      builder: (context) => AlertDialog(
        title: const Text('Supprimer tous les PDF'),
        content: Text('Êtes-vous sûr de vouloir supprimer ${_pdfList.length} PDF ? Cette action est irréversible.'),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(context, false),
            child: const Text('Annuler'),
          ...
          TextButton(
            onPressed: () => Navigator.pop(context, true),
            child: const Text('Tout supprimer', style: TextStyle(color: AppTheme.error)),
          ),
        ],
      ),
    );
    ...
    if (confirm == true) {
      bool toutesReussies = true;
      
      for (final pdf in _pdfList) {
        final success = await widget.databaseService.supprimerPdf(pdf['path']);
        if (!success) {
      ...
      if (toutesReussies) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('${_pdfList.length} PDF supprimés'),
            backgroundColor: AppTheme.success,
          ),
        );
        _chargerPdf();
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: const Text('Certains PDF n\'ont pas pu être supprimés'),
